import type { Profile, ChromeMessage, KeywordGroup } from '../types'
    ...
    try {
      ...
      for (const profile of matchingProfiles) {
        ...
        if (profile.keywordGroups && Array.isArray(profile.keywordGroups)) {
          profile.keywordGroups.forEach((group: KeywordGroup, index: number) => {
            const groupId = `profile-${profile.id}-group-${index}`
            chrome.contextMenus.create(
              {
                id: groupId,
                parentId: profileId,
